...
export class QueryTransformValid {
    ...
    public applyBodyValid(abody: string): string {
        if (Array.isArray(abody) || typeof (abody) !== "object") {
            return "Apply body must be object";
        }

        const len = Object.keys(abody).length;
        if (len !== 1) {
            return "Apply body should only have 1 key, has " + len;
        }
    ...
    public applyRuleValid(rule: any): string {
        if (Array.isArray(rule) || typeof (rule) !== "object") {
            return "Apply rule must be object";
        }
        const len = Object.keys(rule).length;
        if (len !== 1) {
            return "Apply rule should only have 1 key, has " + len;
        }
